...
Each operator is uniquely assigned a subset of the topic's partitions, so that all partitions are covered by the set of operators reasoning.
...
This example has a dedicated consumer operator for each partition. The number of partitions can be set at job submission time via a submission time parameter. In this example, every parallel channel is a separate consisitent region with one trigger operator, so that an operator driven consistent region can be configured.
