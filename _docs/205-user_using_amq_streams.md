...
This document describes how to use the SPL operators of the streamsx.kafka toolkit to connect with Red Hat AMQ Streams.
At the time of writing this article, the latest version of AMQ Streams was 1.2.

# What is Red Hat AMQ Streams

AMQ Streams is an Apache Kafka distribution on OpenShift container platform. It is part of the Red Hat [AMQ messaging suite](https://access.redhat.com/products/red-hat-amq) since October 2018 ([Red Hat Announcement](https://access.redhat.com//announcements/3667151)).
...
From the client perspective, AMQ Streams supports:
...
* External listener for access from applications running outside the OpenShift cluster.
  Dependent on the method how the Kafka service is exposed to the outside, a plain text connection may be supported or not.
...
- **hostname verification**
  ...
  Exposing Kafka as *NodePort* service for external clients, hostname verification must be disabled in the clients when TLS is used.
...
When you have access to the OpenShift environment where AMQ Streams is deployed, you must know how the Kafka cluster is configured, in order to fetch the right information. How to gather the data is described in the [AMQ Streams Documentation](https://access.redhat.com/documentation/en-us/red_hat_amq/7.4/html-single/using_amq_streams_on_openshift_container_platform/index).
...
When storefiles are bundled with the application bundle (SAB), the files should be stored in the `etc` directory of the SPL application folder. The application folder must be denoted in the property with the `{applicationDir}` placeholder, for example

```
ssl.keystore.location={applicationDir}/etc/mykeystore.jks
```

**Note:** When keystore files or truststore files are attached to the application bundle, they cannot be replaced without re-building the bundle and activated them by re-submitting the new bundle as a new job. When they are stored in a shared filesystem, they can be replaced. A restart of the PEs would apply the new keystore or truststore files.
...
```
$ streamsx-kafka-make-properties --bootstrap kafka1.my.domain:443 --trusted-cert cluster-ca.crt \
    --auth TLS --client-cert user.crt --client-private-key user.key --out-propfile kafka.properties
...
This invocation generates the files `keystore.jks` and `truststore.jks` in current working directory, and the property file `kafka.properties` with following content:
...
```
The store files must be copied into the `etc` folder within your application as they are referenced in this way by the properties. The property file itself can be used as basis for separate consumer or producer properties or can be used the way they are.
...
Copy the truststore file, here `./truststore.jks`, to its destination, and configure it in the `ssl.truststore.location` property. Configure the truststore password as `ssl.truststore.password` property.
...
Copy the keystore file, here `./keystore.jks` to its destination, and configure it in the `ssl.keystore.location` property. Configure the keystore password as both `ssl.keystore.password`, and `ssl.key.password` properties. The key has been encrypted with the same password as used to secure the keystore.
...
When the keystore and truststore files are stored on a shared volume, the stores can be replaced there, and the PEs, which host the Kafka operators, must be restarted.
