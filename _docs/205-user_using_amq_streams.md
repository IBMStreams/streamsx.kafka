...
This invocation would generate the files `keystore.jks` and `truststore.jks` in current working directory, and the property file `kafka.properties` with following content:
...
Copy the keystore file, here `./keystore.jks` to its destination folder, and configure it in the `ssl.keystore.location` property. Configure the keystore password as both `ssl.keystore.password`, and `ssl.key.password` properties. The key has been encrypted with the same password as used to secure the keystore.
