...
@PrimitiveOperator(name="KafkaConsumer", namespace="com.ibm.streamsx.kafka",
description="Java Operator KafkaConsumer")
...
public class KafkaConsumerOperator extends AbstractKafkaOperator {
...
//	}
	...
	public static void checkMessageParam(OperatorContextChecker checker) {
		...
		String messageAttrName = paramNames.contains("oututMessageAttrName") ?  //$NON-NLS-1$
				checker.getOperatorContext().getParameterValues("outputMessageAttrName").get(0) : DEFAULT_OUTPUT_MESSAGE_ATTR_NAME; //$NON-NLS-1$
		...
		if(paramNames.contains("outputTopicAttrName")) { //$NON-NLS-1$
			...
			if(topicAttr == null) {
				checker.setInvalidContext(Messages.getString("OUTPUT_ATTRIBUTE_NOT_FOUND", topicAttrName), new Object[0]); //$NON-NLS-1$
			}
		}
	}
	...
	public static void checkTriggerCount(OperatorContextChecker checker) {
		ConsistentRegionContext crContext = checker.getOperatorContext().getOptionalContext(ConsistentRegionContext.class);
		if(crContext != null) {
    ...
    public synchronized void initialize(OperatorContext context)
        ...
        consumer = new KafkaConsumerClient(context, keyClass, valueClass, kafkaProperties, topics, startPosition);
    ...
    public synchronized void shutdown() throws Exception {       
        ...
        OperatorContext context = getOperatorContext();
        logger.trace("Operator " + context.getName() + " shutting down in PE: " + context.getPE().getPEId() + " in Job: " + context.getPE().getJobId() ); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
