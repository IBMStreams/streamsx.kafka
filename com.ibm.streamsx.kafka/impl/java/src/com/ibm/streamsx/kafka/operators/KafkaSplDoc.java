...
public class KafkaSplDoc {
    ...
    public static final String CONSUMER_WHERE_TO_FIND_PROPERTIES = ""
            + "The operator implements Kafka's KafkaConsumer API of the Kafka client version 2.5.1. As a result, it supports all "
            + "Kafka configurations that are supported by the underlying API. The consumer configs for the Kafka consumer 2.5 "
            + "can be found in the [https://kafka.apache.org/25/documentation.html#consumerconfigs|Apache Kafka 2.5 documentation].\\n"
            + "\\n"
    ...
    public static final String CONSUMER_CONNECT_WITH_KAFKA_010 = ""
            + "# Using Kafka server version 0.10"
            + "\\n"
            + "When connecting with Kafka 0.10, following must be considered:\\n"
    ...
    public static final String CONSUMER_INCREMENTAL_COOPERATIVE_REBALANCING = ""
            + "# Incremental Cooperative Rebalancing of Consumer Groups\\n"
            + "\\n"
            + "Kafka 2.4 implemented incremental cooperative rebalancing of consumers, [https://cwiki.apache.org/confluence/display/KAFKA/KIP-429%3A+Kafka+Consumer+Incremental+Rebalance+Protocol|KIP-429]. "
            + "The rebalancing protocol is an *embedded protocol* between the consumer that is chosen as the *group leader*, and the other "
            + "group members to negotiate *who owns which partitions*. The Kafka server, especially the *group coordinator* is not aware of this embedded protocol, "
            + "so that the new rebalancing protocol is available for all Kafka server versions, which are supported by this toolkit.\\n"
            + "\\n"
            + "The new rebalancing strategy is beneficial for large consumer groups and happens in multiple rounds, "
            + "in which only some topic partitions are moved (revoked/assigned) between consumers. Partitions that need not "
            + "be revoked, stay assigned and continue being consumed. A more elaborated description of this new feature can be "
            + "found in [https://www.confluent.io/blog/cooperative-rebalancing-in-kafka-streams-consumer-ksqldb/|this confluent blog post].\\n"
            + "\\n"
            + "**Activation**\\n"
            + "\\n"
            ...
            + "(stop-the-world effect). These are the built-in eager assignors, which can be configured:\\n"
            + "* org.apache.kafka.clients.consumer.RangeAssignor\\n"
            + "* org.apache.kafka.clients.consumer.RoundRobinAssignor\\n"
            + "* org.apache.kafka.clients.consumer.StickyAssignor\\n"
            + "\\n"
            + "**Note:** When the consumer operator is used in a consistent region, automatically an assignor for the EAGER protocol is configured "
            + "as every rebalance iteration would trigger a reset of the consistent region. The reset of a consistent region is a *stop-the-world* event "
            + "for the application so that consumers in a consistent region would not benefit from incremental rebalancing. "
            + "On the contrary, incremental rebalancing could have a negative impact in consistent region.\\n"
            + "";
    ...
    public static final String CONSUMER_DEFAULT_AND_ADJUSTED_PROPERTIES = ""
            ...
            + "|---|\\n"
            + "| partition.assignment.strategy | **When in consistent region**, the partition assignment strategy is adjusted to strategies that support the EAGER protocol only. "
            + "When not set or no EAGER only assignor remains, and in consistent region, "
            + "`org.apache.kafka.clients.consumer.RangeAssignor` or `org.apache.kafka.clients.consumer.RoundRobinAssignor` "
            + "is chosen dependent on number of subscribed topics. **When not in a consistent region and no strategy is given:** "
            + "`org.apache.kafka.clients.consumer.RoundRobinAssignor` when multiple topics or a pattern is subscribed. "
            + "Otherwise Kafka's built-in default applies. |\\n"
            + "|---|\\n"
    ...
    public static final String CONSUMER_KAFKA_GROUP_MANAGEMENT = ""
            ...
            + "\\n"
            + "The operator is capable of taking advantage of [https://kafka.apache.org/23/documentation/#intro_consumers|Kafka's group management function].\\n"
    ...
    public static final String CONSUMER_STATIC_GROUP_MEMBERSHIP = ""
            ...
            + "\\n"
            + "Since version 2.3, Kafka supports [https://kafka.apache.org/25/documentation/#static_membership|static group membership], "
    ...
    public static final String PRODUCER_WHERE_TO_FIND_PROPERTIES = ""
            ...
            + "The operator implements Kafka's KafkaProducer API of the Kafka client version 2.5.1. As a result, "
            + "it supports all Kafka properties that are supported by the "
            + "underlying API. The producer properties for the Kafka producer 2.5 "
            + "can be found in the [https://kafka.apache.org/25/documentation/#producerconfigs|Apache Kafka 2.5 documentation].\\n"
            + "\\n"
            + "When you reference files within your application, which are bundled with the Streams application bundle, for example "
            + "an SSL truststore or a key tab file for Kerberos authentication, you can use the `{applicationDir}` placeholder in the "
            + "property values. Before the configs are passed to the Kafka client, the `{applicationDir}` placeholder is replaced "
    ...
    public static final String PRODUCER_CONNECT_WITH_KAFKA_010 = ""
            + "# Using Kafka server version 0.10"
            + "\\n"
            + "When connecting with Kafka 0.10, following must be considered:\\n"
            + "* The **consistentRegionPolicy** parameter cannot be `Transactional`.\\n"
            + "* The **guaranteeOrdering** parameter cannot be `true`. To guarantee ordering, the producer config "
            + "  `max.in.flight.requests.per.connection=1` must be used.\\n"
            + "\\n"
            + "";
    ...
    public static final String PRODUCER_DEFAULT_AND_ADJUSTED_PROPERTIES = ""
            ...
            + "|---|\\n"
            + "| transactional.id | Randomly generated ID in the form: `tid-<random_string>` only when in consistent region and the **consistentRegionPolicy** parameter is set to `Transactional`. |\\n"
            + "|---|\\n"
            ...
            + "specified for the Kafka producer version 2.5, are supported by all Broker versions. An example for such a config is the Zstandard "
